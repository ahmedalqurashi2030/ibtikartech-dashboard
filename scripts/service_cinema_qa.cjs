...
(async()=>{
 ...
 if(process.env.QA_INSPECT){
  if(process.env.QA_ROUTE){await send('Page.navigate',{url:'http://127.0.0.1:8000'+process.env.QA_ROUTE});await wait(700);}
 ...
 const findings=[];
 for(const width of [375,768,1024,1440,720]){
  await send('Emulation.setDeviceMetricsOverride',{width,height:width===720?375:960,deviceScaleFactor:1,mobile:false});
  for(const route of routes){
   await send('Page.navigate',{url:'http://127.0.0.1:8000'+route});await wait(650);
   await ev(`document.querySelectorAll('.reveal').forEach(e=>e.classList.add('visible','is-visible'));`);
   const result=await ev(`({overflow:document.documentElement.scrollWidth>innerWidth+2,h1:document.querySelectorAll('h1').length,paths:document.querySelectorAll('.service-path-card').length,scenes:[...document.querySelectorAll('.service-path-card')].every(e=>!!e.dataset.scene),broken:[...document.images].filter(i=>i.complete&&i.naturalWidth===0).map(i=>i.getAttribute('src'))})`);
   findings.push({width,route,...result});
   if(result.overflow) console.log('OVERFLOW',width,route);
   if(width===1440&&route==='/ecommerce/'){
    ...
    const shot=await send('Page.captureScreenshot',{format:'jpeg',quality:65});fs.writeFileSync(path.join(output,'ecommerce-desktop.jpg'),Buffer.from(shot.data,'base64'));
   ...
   if(width===375&&route==='/ecommerce/'){
    ...
    const shot=await send('Page.captureScreenshot',{format:'jpeg',quality:65});fs.writeFileSync(path.join(output,'ecommerce-mobile.jpg'),Buffer.from(shot.data,'base64'));
   }
  }
 }
 await send('Emulation.setScriptExecutionDisabled',{value:true});
 await send('Page.navigate',{url:'http://127.0.0.1:8000/services/store-launch/'});await wait(400);
